...
function assignNotesToStrings(noteOptions, prevFret = null) {
  ...
  function scoreAssignment(assignment) {
    ...
    const span = fretted.length > 1
      ...
      : 0;
    // Count notes on E string that could have gone on A with fret ≤ 14
    let avoidableECount = 0;
    for (let i = 0; i < assignment.length; i++) {
      const a = assignment[i];
      if (a !== null && a.string === "E") {
        const hasA = noteOptions[i].options.some(
          (o) => o.string === "A" && o.fret <= 14
        );
        if (hasA) avoidableECount++;
      }
    }
    ...
    return { withinSpan: span <= MAX_FRET_SPAN, strings: stringsUsed, avoidableECount, melodyDistance, openCount, fretSum };
  }
  ...
  function isBetter(score) {
    ...
    // 3. Minimize avoidable E-string usage (prefer A over E)
    if (score.avoidableECount < bestScore.avoidableECount) return true;
    if (score.avoidableECount > bestScore.avoidableECount) return false;
    // 4. Prefer open strings
